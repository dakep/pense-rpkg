...
test_that("PENSE Algorithm (1 thread)", {
  ...
  skip_if_not(nzchar(Sys.getenv('PENSE_TEST_FULL')), message = 'Environment variable `PENSE_TEST_FULL` not defined.')
  ...
  pr <- pense(x, y, alpha = 0.8, nlambda = nlambda, nlambda_enpy = 5, ncores = 1L,
              sparse = FALSE, enpy_opts = enpy_options(retain_max = 5, en_algorithm_opts = en_lars_options()),
  ...
  if (!file.exists(snapshot_file)) {
    jsonlite::write_json(pr$estimates, path = snapshot_file, auto_unbox = TRUE, digits = 14, pretty = TRUE)
  ...
  for (i in seq_along(ref_estimates)) {
    for (name in names(ref_estimates[[i]])) {
      expect_success(expect_equal(drop(pr$estimates[[!!i]][[!!name]]), unlist(ref_estimates[[!!i]][[!!name]])))
    }
  }
})
...
test_that("PENSE Algorithm (2 threads)", {
  ...
  x <- matrix(rcauchy(n * p), ncol = p)
  y <- 2 + rowSums(x[, 1:5]) / 5 + rnorm(n, sd = 4)
  pr <- pense(x, y, alpha = 0.8, nlambda = nlambda, nlambda_enpy = 5, ncores = 2L,
              sparse = FALSE, enpy_opts = enpy_options(retain_max = 5, en_algorithm_opts = en_lars_options()),
              algorithm_opts = mm_algorithm_options(en_algorithm_opts = en_lars_options()))
